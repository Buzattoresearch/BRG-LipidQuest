...
def calculate_sensitivity_score(polarity, adduct, lipid_class, sensitivity_dict):
    try:
        score = float(sensitivity_dict.get(polarity, {}).get(adduct, 1))     
        # Penalty due to presence in the LMSD library (lipids that have been detected in nature)
            
        if lipid_class in ['M(IP)2C', 'PnC', 'DGTA', 'PnE', 'PGS', 'PT', 'PS-NAc', 'NAPE', 'SLBPA', 'PPA', 'PE-IsoK', 'LSM', 'PGP', 'DGTS', 'GlcADG', 'GP', 'Glc-GP', 'DGMG', 'MGMG', 'DGCC']: # <= 0.1% of LMSD entries
            penalty += 0.9

        elif lipid_class in ['SQDG', 'DGDG', 'SQMG', 'SPBP', 'HexSPB', 'PIP', 'SCer']: # 0.2% of LMSD entries
            penalty += 0.75

        elif lipid_class in ['SHexCer', 'CerP', 'FAG', 'PE-NMe', 'NAT']: # <= 0.05% of LMSD entries
            penalty += 0.5

        elif lipid_class in ['CDP-DG', 'LPG', 'MGDG', 'LPI']: # <= 0.10% of LMSD entries
            penalty += 0.25

        elif lipid_class in ['LPIM', 'LPA', 'PI-Cer', 'NAE', 'LPS', 'MG', 'SPB']: # <= 0.20% of LMSD entries
            penalty += 0.10

        elif lipid_class in ['TG', 'FA', 'PK', 'ST', 'HexCer']:  # >5% of LMSD entries (too many entries)
            penalty += 0.10


        # Penatly due to uncommon lipids in mammalian samples        
        if lipid_class in ['PMeOH', 'PEtOH', 'WE', 'FAL', 'FOH']:
            penalty += 0.9

        if lipid_class in ['PK', 'PR', 'SL', 'Other', 'GP', 'DGTS', 'HexSPB', 'NAT', 'SHexCer', 'SQDG', 'SMGDG', 
                        'CerP', 'MIPC', 'M(IP)2C', 'SHexCer', 'SCer', 'HC', 'PnC', 'PnE', 'FOH', 'FAL', 'LSM',
                        'CDP-DG', 'GlcADG', 'DGCC', 'SulfateHexSPB', 'MGMG', 'DGMG', 'MGDG', 'DGDG', "SL", "PIM", "PI-Cer", "HBMP", "ACer", "Acer", "PE-Cer", 'NAE', 'PIP', 'PE-Cer']:
            penalty += 0.9    

        elif lipid_class in ['LPA', 'LPS', 'LPI', 'NA', 'SPB', 'ACer', 'CoA', "PK", "PR", "SHexCer", "CerP", "ST"]:
            penalty += 0.80

        elif lipid_class in ['PA', 'CL', 'PG']:
            penalty += 0.35

        elif lipid_class in ['PS', 'PI', 'BMP', 'CAR', 'MG']:
            penalty += 0.20
...
def calculate_fa_chain(number_of_carbons, lipid_class): # even or odd
...
def calculate_plasmenyl_score(has_plasmenyl, lipid_class):
    if has_plasmenyl == "Yes":
        if lipid_class in ['PC', 'PE']:
            return 0.5
        if lipid_class in ['PG', 'PA', 'PS', 'PI', 'LPE', 'LPC', 'LPG', 'LPA', 'LPI']:
            return 0.75
        return 1
    return 0
...
def calculate_modifications_score(n_modifications, lipid_class, modifications, name):
    ...
    if any(x in str(modifications) for x in ['S','Br','Cl','As','Si','F','I','T','G']):
        score = 1
    if any(x in str(name) for x in ['medication','contaminant', 'marine', 'plant', 'NoAbbrev']):
        score = 1
    return score
